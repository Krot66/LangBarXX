...
<p align="center">
<img src="ReadMe.assets/ReadMe9.png" alt="ReadMe6" width="500" />
</p>
...
- *Тултип автозамены с нажатым ScrollLock* - при нажатом ScrollLock после автозамены будет выводится две секунды тултип с указанием ее номера и сработавшего сочетания знаков. Выводится над текстовым курсором или, если его положение не определено, в центре экрана.
...
Данные три опции представляются в главном окне в виде нолей или единиц в соответствующей колонке списка правил.
...
Если вы вставляете какую-то конструкцию, и после вам нужно переместить текстовый курсор на сколько позиций назад, для этого можно использовать сочетание вида `{Left n}`, где т - число нажатий на курсовую клавишу *Left*. Например, замена
...
Формат буфера не ограничен офисными документами. Можно вставлять все, что вообще возможно копировать. Распаковка сохраненного буфера обмена размером в несолько десятков мегабайт производится почти мгновенно, так что все определяется возможностями программы, в которую вы будете его вставлять.
...
Кнопка *Предпросмотр* предназначена для того, чтобы увидеть вводимый текст с их значениями переменных, имеющимися на данный момент.
...
Номера заданий не должны иметь пропусков, и если он возник, все последующие задания будут игнорироваться.
