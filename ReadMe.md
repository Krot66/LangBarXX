...
## 8.2 Форматирование горячими клавишами
...
будет форматировать 1-й заголовок за счет посылки сочетания Ctrl+1 (здесь ^{1}) и далее вводить простой текст с сочетанием Ctrl+0 (^{0}).
...
будет вставлять текст `08.12.2023  16:52`. 
...
## 8.6 Ручное редактирование автозамен
...
В начале каждой строки находится чекбокс, если отметить который по нажатию кнопки OK флажок будет заменен на текстовый с коротким названием языка. Поскольку для каждого языка существует множество разновидностей (например, американский и британский английский), имя флажка будет браться из последних, “локальных” букв ее сокращенного обозначения.

Чтобы сменить цвет на предустановленный, надо дважды щелкнуть по интересующей строке или нажать на ввод прежде выделив ее мышью (можно выбрать первый пункт из контекстного меню). Откроется стандартный диалог выбора цветов, в котором можно настроить нужный. Если отсутствует флажок для нужной раскладки, текстовый флажок будет выбран автоматически. В случае неопределенной раскладки на флажке будут отображаться два вопросительных знака.
...
Уведомление о переключении раскладки представляет собой тот же текстовый флажок, только имеющий некоторые самостоятельные настроки в другой секции:

```ini
[LangNote]
Note=1 ; включенность уведомлении (чекбокс в настройках)
Note_Size=5 ; размер в процентах к размеру экрана
...
При отсутствии прав администратора программа устанавливается в папку %LOCALAPPDATA% или Application Data на Windows XP/2003. Другой вариант - пользоваться портативной версией. 
