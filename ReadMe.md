...
Индикатор всегда выключен если активным окном являются рабочий стол или панель задач. Так же индикатор всегда выключен при активации полноэкранного приложения, если в меню не отмечен пункт “На полном экране.”
...
Если открыть каталог программы, там есть папка *dict*, а ней подпапки с короткими именами языков, такими же, как используются для флажков в каталоге *flags*. Внутри имеются файл или файлы с расширением *dic*, взятом от словарей *Hunspell*, на основании которых они были сгенерированы. Все файлы содержащиеся в папке обрабатываются программой, их можно модифицировать или дополнять, о чем будет сказано ниже. Имена файлов не имеют значение, но есть один, имеющий название папки, который считается основным (en-US.dic для английского, ru-RU.dic для русского и т. д.), что нужно для возможности отключения дополнительных словарей в процессе отладки, а также для обновления при установке новой версии (дополнительные словари при этом остаются на месте). Заметьте, в названиях некоторых словарей Hunspell вместо дефиса может использоваться нижняя черта!
...
- Режим одного языка (одностороннее преобразование к одному языку) - *Ctrl+Alt+Ctrl*. Зеленая точка внизу флажка, индикатора и иконки в трее.
- Для отмены автоматического переключения достаточно нажать <u>левый</u> *Shift+Bacspace* (работает и после одного пробела после слова) или *Shift+Pause*
- Для отмены преобразования можно использовать и сочетание *Ctrl+Z* (если включено в настройках)
...
- Пользовательский словарь - простой текстовый файл (user_dict.dic в каталоге настроек) в кодировке utf-8, куда записываются слова, преобразование которых запрещено, по одному на строку, причем возможны слова на разных языках. Словарь автоматически учитывается при каждом сохранении текста, так что перезапуск программы не требуется. Лучше помещать туда неизменную часть слов, или, если необходимо, несколько словоформ.
...
- Уведомление при автопереключении - выводит стандартное уведомление при автопереключении. На Windows 10 - 11 выезжающее сбоку, на более старых операционных системах имеет вид балонной подсказки в трее. Уведомление беззвучно и совместимо с предыдущей опцией. Работает, если уведомления не отключены для LangBar++ в настройках.

- Отмена преобразования по Ctrl+Z - позволяет использовать стандартное сочетание клавиш для отмены преобразования. Работает однократно - далее все происходит как в используемом приложении, которое вообще может и не иметь иметь такую горячую клавишу.

- Языки автопереключения - меню пар языков установленных в системе, для которых имеются словари в каталоге dict

- Только основной словарь - использовать только основной словарь при автопереключении (если вы не соблюдали правила именования словарей, не будет загружен ни один словарь!)

- Режим одного языка - содержит меню предпочитаемого языка, на который будет проводится автопереключение и лист языков с чекбоксами, при наборе на которых будет оно возможно. Учитываются лишь языки, к которым имеются словари. Выбор большого числа языков может замедлить готовность программы после запуска.

- Загружать словари из папки dict_plus - дополнительные словари для режима одного языка (смотри ниже)
...
- Игнорировать акценты - считает ё=е, й=и, á=a и так далее при сравнении набранных слов со словарями. Снятие этой опции требует строгой печати и, что гораздо сложнее, соответствующих словарей. При этом игнорирование акцентов относится лишь к сравнению - преобразованное слово имеет акценты нажатых вами клавиш!

- Обработка аббревиатур

    1. Сравнение с учетом регистра - проверяет, существует ли такое слово в верхнем регистре в словарях. Если вы набираете в верхнем регистре обычные слова, они не проверяются!
    2. Без учета регистра - преобразование происходит при наличии слова в словаре в любом регистре. Позволяет обходить изъяны словарей, где регистр не соблюдается и всегда проверять слова, набранные в верхнем.
    3. Игнорировать полностью - аббревиатуры не загружаются со словарей, а набранные большими буквами слова не проверяются
    4. Игнорировать короткие аббревиатуры - имеется ввиду аббревиатуры длиной в два символа, которыми забиты английские словари в виде сокращенных названий штатов и пр.. Необходимая опция при обработке аббревиатур без учета регистра!

    Как и в предыдущем случае, все сказанное относится только к сравнению со словарями и реальный регистр букв сохраняется. Опция относится и к словам в пользовательском словаре! При загрузке словаря отдельные заглавные буквы всегда преобразуются в нижний регистр и не рассматриваются как аббревиатуры.

- Начальные границы слов

    1. Символы - перечень символов, которые не будут считаться частью слов при преобразоваии. Например, `(цу  ` будет преобразовано в `(we`. Допускается использование символов, на клавишах которых не располагаются буквы в обеих раскладках, поскольку иначе это нарушит автопереключение!
...
Взять их можно здесь: [словари LibreOffice](https://drive.google.com/drive/folders/1Xe13v0hm3nzipRkTSIcbuAaTZGIYYpP2?usp=drive_link), [словари Softmaker FreeOffice](https://www.freeoffice.com/ru/download/dictionaries). В первом случае архив может содержать набор словарей для разных диалектов, в последнем sox-файл надо открыть в архиваторе и распаковать. Они отличаются числом личных имен, аббревиатур, сленга и использованием акцентов. Еще один важный момент, это более развитый механизм аффиксов (приставок, суффиксов и окончаний), расположенных за косой чертой,  у сравнительно новых словарей, уменьшающих общее число слов. В этом отношении старые словари могут быть лучше новых. Программа предоставляет возможность слияния словарей, и некоторые словари сделаны именно таким способом.

Общие требования к словарю программы предельно просты:

- Расширение - dic
- Кодировка - utf-8
- Одно слово на строку
- Естественно, отсутствие слов на других языках и разного рода мусора
- Ведущие и хвостовые пробелы, пустые строки, вид используемых переносов и наличие дубликатов значения не имеют

Исправление раскладки происходит в среднем после 3-4 введенных символов слова. С таким подходом, очевидно, можно не заморачиваться с поиском всех имеющихся форм слов и ограничиться их начальной частью. Можно разобрать на слова достаточно объемный текст и пользоваться им, если ваш словарный запас совпадает с источником или не выходит за его пределы. Если вы пишите как Лев Толстой, можно распилить “Войну и Мир” на примерно 56 000 словоформ и этого будет достаточно. Если нет, нужно делать дополнительные тематические словари и добавить их в каталог программы. Для минимазации, доводки и слияния словарей Hunspell, а также генерации их из произвольного текста, был сделан конвертер словарей, открываемый из меню Автопереключение.
...
Ниже идет выбор способа сохранения - в словарь программы или в отдельный файл.  В первом случае будет открыто окно выбора папки в каталоге dict программы. Вам потребуется выбрать существующую папку  или создать новую с именем существующей локали. Словарь будет сохранен в данной папке как основной, а если он там уже существует, как дополнительный с суффиксами вида `_1`, `_2` и т. д.. Во втором случае откроется окно сохранения файла с именем, использующим временную метку. 

Далее идут кнопки для вставки текста из буфера обмена, через окно открытия или перетаскивания файла (в единственном или множественном числе) на нижнюю кнопку окна. Кодировка utf-8 в последнем случае необходима. Лишь самые новые словари Hunspell имеют юникодную кодировку, в более старых она может быть какой угодно. Если с этим возникли проблемы, откройте одноименный словарю файл с расширением  aff - она будет там в первой строке.
...
- Обрезка слов - нужна для уменьшения размеров словаря и ускорения  обработки и поиска слов на старых машинах. Длина слова не более 6-8 символов кажется вполне достаточной для обычных нужд. Содержимое словарей Hunspell начиная с косой черты обрезается автоматически.
...
Что еще можно использовать в качестве источника словарей? Например, словари StarDict. Имеются всевозможной тематики, в этот формат конвертировано и множество сторонних словарей. Заархивированы в два или три слоя, нужно извлечь в архиваторе текстовый файл с расширением dict. Далее его следует обработать как простой текст, включить фильтрацию по языкам (или удалить лишнее после), и нужная выжимка из словаря у вас в руках!
...
В начале обеих строк идут сокращенные названия раскладок, основной и альтернативной, далее идет текст, набираемый в основной и альтернативной раскладке, затем найденный текст в словарях, здесь английском и русском. Автоматическое переключение при этом не производится чтобы вы могли досмотреть все до конца. Выключение данного режима производится повторным нажатием на флажок с зажатым *Ctrl* или перезапуском программы. При вводе вы видите то, что соответствует настройкам автопереключения, включая обработку акцентов и аббревиатур! Следует также иметь ввиду, что длины слов словаря может не хватить для отображения слова целиком.

Приведенный текст соответствует воду слова “еще” с конечным пробелом в английской раскладке. Очевидно, преобразованию препятствует слово “tot” в английском словаре, которое, возможно, следует закомментировать или удалить. При этом при наборе слова “ещё” (с акцентом) проблем не возникнет, поскольку ему будет соответствовать в английской раскладке сочетание “to`”.

# 6. Изменение регистра, транслитерация
...
# 7. Особенности работы в приложенях
...
## 7.2 FAR, ConEmu, командная строка и терминал
...
# 9. Установка и обновление программы, USB-версия
...
Иначе, если у вас есть настроенная портативная версия программы, достаточно поместить в ее каталог инсталлятор и батник тихой установки для быстрого переноса ее на другой компьютер.
