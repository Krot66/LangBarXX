...
Если открыть каталог программы, там есть папка *dict*, а ней подпапки с короткими именами языков, такими же, как используются для флажков в каталоге *flags*. Внутри имеются файл или файлы с расширением *dic*, взятом от словарей *Hunspell*, на основании которых они были сгенерированы. Все файлы содержащиеся в папке обрабатываются программой, их можно модифицировать или дополнять, о чем будет сказано ниже. Имена файлов не имеют значение, но есть один, имеющий название папки, который считается основным (en-US.dic для английского, ru-RU.dic для русского и т. д.), что нужно для возможности отключения дополнительных словарей в процессе отладки, а также для обновления при установке новой версии (дополнительные словари при этом остаются на месте). Заметьте, в некоторых словарях Hunspell вместо дефиса может использоваться нижняя черта!
...
<p align="center">
<img src="ReadMe.assets/ReadMe7.png" alt="ReadMe7" width="300" />
</p>
...
Взять их можно здесь: [словари LibreOffice](https://drive.google.com/drive/folders/1Xe13v0hm3nzipRkTSIcbuAaTZGIYYpP2?usp=drive_link),  [словари Softmaker FreeOffice](https://www.freeoffice.com/ru/download/dictionaries). В первом случае архив может содержать набор словарей для разных диалектов, в последнем sox-файл надо открыть в архиваторе и распаковать. Они отличаются числом личных имен, аббревиатур, сленга и использованием акцентов (еще аффиксов за косой чертой, но программа эту часть строк игнорирует). Сказать что лучше загодя сложно - нужно смотреть самому. Некоторые словари программы сделаны слиянием обеих версий.
...
- Обрезка слов - нужна для уменьшения размеров словаря и ускорения  обработки и поиска слов на старых машинах. (Русский соварь за счет удаления возникающих дубликатов уменьшается более чем вдвое по числу слов и вчетверо по размеру файла.) Длина слова не более 6 символов кажется вполне достаточной для обычных нужд. Содержимое словарей Hunspell начиная с косой черты обрезается автоматически. При сходстве языков (белорусский-русский) такая длина слов словаря может оказаться недостаточна!
...
Словари загружаются при запуске программы, поэтому для применения изменений или добавления словарей следует перезапустить программу.

Что еще можно использовать в качестве источника словарей? Например, словари StarDict. Имеются всевозможной тематики, в этот формат конветировано и множество сторонних словарей. Заархивированы в два или три слоя, нужно извлечь в архиваторе текстовый файл с расширением dict. Далее его следует обработать как простой текст, включить фильтрацию по языкам (или удалить лишнее после), и нужная выжимка из словаря у вас в руках!
...
Так же там есть файл CapsLock.png, предназначенный для изменения цвета тени флажка, если он вас не устраивает. Он масштабируется до размера флажка при отображении, его размеры значения не имеют. Файл Autocorrect.png является той красной точкой, что отрисовывается на иконке и флажках при автопереключении.
...
*В. Индикатор раскладки не перемещается на другой монитор - что с этим делать?*
О. Отображение возможно только на основном дисплее - нужно сделать его основным в настройках экрана и перезапустить программу.
