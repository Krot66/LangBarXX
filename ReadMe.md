...
## 1.2 На текстовом курсоре

Когда курсор мыши имеет вид двутавра (IBeam), какой он приобретает над полями текстового ввода, на курсоре отображается флажок, если включен соответствующий пункт в меню *Флажок курсора.* В основном его значение в том, что есть программы, в которых положение каретки не определяется, и вряд ли когда-то будет. Кроме того, он показывает текущую раскладку до в большинстве случаев необходимого клика мышью. Сделано отображение текстового флажка в Windows Terminal, где курсор всегда имеет вид стрелки.
Размер флажка настраивается в соответствующем пункте того же меню. Рекомендуется выключить отображение тени курсора в *Панели управления - Мышь* для лучшего его отображения. Флажок курсора имеет лучший вид при использовании картиночных, нетекстовых флажков.
...
В левом верхнем углу находится поле ввода сокращения, причем для ясности через пробел можно вписать короткий комментарий или расшифровку многоязычной автозамены (например, `tot еще`).
...
Автозамена текста работает вне зависимости от автоматического исправления раскладки и на нее не действуют настройки в правилах приложений. Кроме того, она срабатывает мгновенно и всегда опережает исправление раскладки, для которого нужны некоторые вычисления.
...
Файлы сохраняются до тех пор, пока имеются использующие их автозамены. Они удаляются в корзину при закрытии окна после удаления соответствующей автозамены. <u>Поэтому если вы меняете файл автозамен hotstrings.ini и хотите потом вернуться к старому, следует переименовать и папку Clips!</u> Понять, какой файл к чему относится можно по цифрам: это год, месяц, день и время без разделительных знаков. Файлы считываются в момент вставки и не висят в памяти постоянно, их можно иметь сколько угодно.

> При использовании таких автозамен имеет смысл выставить точное соответствие в настройках. Для того, чтобы быстро находить нужное в списке автозамен, можно ввести в начале, перед именем файла в папке Clips, или после него комментарий - это не повлияет на его выполнение.
...
ConEmu является наиболее беспроблемным инструментом на всех осях, позволяющим отображать флажок и преобразовывать выделенный текст.
...
Как и в предыдущем случае, все восстанавливается по умолчанию при удалении.
