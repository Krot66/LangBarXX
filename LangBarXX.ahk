...
IniRead lctrl, % cfg, Layouts, LCtrl, % " "
IniRead rctrl, % cfg, Layouts, RCtrl, % " "
IniRead lshift, % cfg, Layouts, LShift, % " "
IniRead rshift, % cfg, Layouts, RShift, % " "
...
If (lang_select=1)
    Loop  % lang_count
        Hotkey % "<^vk3" A_Index, SetInputLang
...
KeyArray(hook, vk) {
    ...
    ks.Push([prefix "{" Format("vk{:x}", vk) "}", GetKeyState("CapsLock", "T")])
...
Select:
    If WinActive("ahk_class VMPlayerFrame") || WinActive("ahk_exe VirtualBox.exe") || WinActive("ahk_exe VirtualBoxVM.exe") || ((A_PriorHotkey=A_ThisHotkey) && (A_TimeSincePriorHotkey<50))
        Return
    ...
    If StrLen(text) {
        If (button~="(RButton|MButton)")
            SetTimer Flag, Off
        If WinActive("ahk_class ConsoleWindowClass") || WinActive("ahk_class VirtualConsoleClass") || WinActive("ahk_exe WindowsTerminal.exe") || ((A_ThisHotkey~="BS$") && !pause_shift_bs) || ((A_ThisHotkey~="Pause") && pause_shift_bs) || (A_ThisHotkey~=">\+(F|vk)")
            send_bs:=1
        If symbsel {
...
ReConvert:
    ...
    Loop Parse, sel
    {
        ...
        If (vk="vk20d") ; удаление двойных переносов
            continue
        If (vk~="vkfff") {
            If (lang_count=2) && (A_ThisLabel="Convert") && (button~="(Pause|BS|RButton|CapsLock|vk50)") {
                SetInputLang(1)
                Goto Reconvert
            }
            ToolTip Неверная`nраскладка!,  % x-40, % y-50
            SetTimer ToolTip, -2000
            SetInputLang(1, lang_sel)
            Exit
        }
        convert.="{" vk "}"
...
SendText(txt) {
    global vk_string ; Для слежения и записи лога
    If !txt
        Return
    If IsObject(txt) {
        vk_string:=""
...
#If OnFlag(FlagHwnd) && (InputLayout()~="(" pause ")") && pause
RButton::Goto Translate
...
Translate:
    ...
    Sleep 50
    SendText(out)
    ;FileAppend % A_Now " " hkey " " A_ThisHotkey " " InputLayout() "`r`ntext: " text "`r`nsel: " sel "`r`nconvert: " convert "`r`nvk_string: " vk_string " (" txt.Length() ")`r`n`r`n", Log.txt, UTF-8 ; логирование введенного и обработанного текста
    Return
...
SetInputLang:
    ...
    StringCaseSense Off
    If (A_ThisHotkey~="<\^?vk\d+") || (A_ThisHotkey~="<!vk\d+") {
        KeyWait % RegExReplace(A_ThisHotkey, ".+(?=vk)"), T1
        KeyWait LCtrl, T1
        KeyWait LAlt, T1
        Sleep 50
        SetInputLang(1, lang_array[SubStr(A_ThisHotkey, 0), 1])
        Return
        ;target:=lang_array[SubStr(A_ThisHotkey, 0), 1]
    }
    If (A_ThisHotkey~="(F|vk)\d+$")
        target:=lang_array[SubStr(A_ThisHotkey, 0),1]   
    Else If (A_ThisHotkey~="^(RButton|Pause)$") || ((A_ThisHotkey="Capslock") && (capslock=3))
...
LShift up::
    ih.Stop()
    SetInputLang(1, lshift)
    Return

#If rshift
RShift up::
    SetInputLang(1, rshift)
    Sleep 200
    ih.Stop()
    Return
...
LCtrl up::
    ih.Stop()
    SetInputLang(1, lctrl)
    Return

#If rctrl
RCtrl up::
    SetInputLang(1, rctrl)
    Sleep 200
    ih.Stop()
    Return
...
~*LButton::
    ...
    If WinActive("ahk_class MozillaWindowClass")
        ControlFocus MozillaCompositorWindowClass1
~*MButton::
    ih.Stop()
    Return
    ...
    If ((app_state>0) || (indicator && !(app_state=0)) || WinExist("ahk_id" Gui5)) {
        ...
        If upd || !WinExist("ahk_id" IndHwnd) || (lang && (lang!=lang_in_old)) || (caps!=caps_in_old) || (num!=num_in_old) || (scr!=scr_in_old) {
            ...
            If !no_border {
                ...
                Gdip_FillRectangle(G ,Brush, -1, w_in-h_in-1, w_in+1, h_in+1)
...
Flag:
    ...
    If flag {
        If (lang && (lang!=lang_fl_old)) || (width!=width_old) || !WinExist("ahk_id" FlagHwnd){
            pFlag:=Gdip_CreateBitmapFromFile(flag_png)
            Gdip_GetImageDimensions(pFlag, wf, hf)
            fl_h:=file_aspect ? width*hf//wf : width*3//4, mn:=no_border ? 0 : ((width>48) ? 2 : 1)
            pBitmap:=Gdip_CreateBitmap(width+mn*2, fl_h+mn*2)
            G:=Gdip_GraphicsFromImage(pBitmap)
            Gdip_SetSmoothingMode(G, smoothing)
            Gdip_SetInterpolationMode(G, scaling)
            If !no_border {
                Brush:=Gdip_BrushCreateSolid(no_border ? 0xFF : 0x33000000)
        ...
        If _x && _y && (FlagHwnd!=WinExist("A")){
            ...
            ;WinSet Redraw,, ahk_id %FlagHwnd%
            If !WinExist("ahk_id" FlagHwnd)
                Gosub FlagGui
            width_old:=width, lang_fl_old:=lang, fl_h_old:=fl_h
        }
        Else
            Gui Hide
    }
...
LayoutsAndFlags:
    ...
    row:=0, lang_array:=[], uflag:=0, lang_index:=""
    ...
    lang_menu:="Выкл.", lang_index:=RegExReplace(lang_index, "*$"), lshift_ind:=rshift_ind:=lctrl_ind:=rctrl_ind:=1
    Loop % lang_array.Length() {
        lang_menu.="|" lang_array[A_Index, 2]
        If (lshift=lang_array[A_Index, 1])
            lshift_ind:=A_Index+1
        If (rshift=lang_array[A_Index, 1])
            rshift_ind:=A_Index+1
        If (lctrl=lang_array[A_Index, 1])
            lctrl_ind:=A_Index+1
        If (rctrl=lang_array[A_Index, 1])
            rctrl_ind:=A_Index+1
    }
    lshift:=(lshift_ind>1) ? lshift : "", rshift:=(rshift_ind>1) ? rshift : "", lctrl:=(lctrl_ind>1) ? lctrl : "", rctrl:=(rctrl_ind>1) ? rctrl : ""
    ...
    Gui 3:Add, GroupBox, x16 w616 h170, Переключение раскладки
    ...
    Gui 3:Add, CheckBox, x40 yp+40 vkey_switch, Использовать имитацию клавишного переключения раскладки

    Gui 3:Add, GroupBox, x20 w616 h140, Исправление раскладки
    ...
    Gui 3:Add, GroupBox, x16 yp+44 w616 h66, Работа с множественными раскладками (+ правые Ctrl и Shift)
...
3Save:
    ...
    Loop 4
        If rb%A_Index%
            lang_select:=A_Index
    lshift:=(lshift_ind>1) ? lang_array[lshift_ind-1, 1] : ""
    rshift:=(rshift_ind>1) ? lang_array[rshift_ind-1, 1] : ""
    lctrl:=(lctrl_ind>1) ? lang_array[lctrl_ind-1, 1] : ""
    rctrl:=(rctrl_ind>1) ? lang_array[rctrl_ind-1, 1] : ""
